...
Upgrading a kubeadm cluster and backing up/restoring etcd are frequently covered in the CKA exam. Be sure to practice both several times before the test.
{{< /hextra/callout >}}
...
{{< hextra/callout type="warning" >}}
At the end of this section, you'll need to upgrade the cluster you've created at the beginning of this workshop. Make sure you run the right commands from the right Node, and everything will be fine :)
{{< /hextra/callout >}}
...
```yaml {filename="/etc/kubernetes/manifests/etcd.yaml"}
 …
 volumes:
 - hostPath:
