...
When we create a cluster, an admin **kubeconfig** file is genetated, similar to the following one.

```yaml {filename="/etc/kubernetes/admin.conf"}
apiVersion: v1
clusters:
- cluster:
...
![ControlPlane](./images/control-plane.png)

The **/etc/kubernetes** folder contains the following files to ensure this communication is secured.

- kubeconfig files to authenticate internal components againts the API Server
...
This folder also contains the YAML manifests defining static Pods; all of them are in **/etc/kubernetes/manifests**. More on this soon.
{{< /hextra/callout >}}
...
For information purposes, the following table gives the subject of the certificates embedding in each kubeconfig file.
...
![RBAC Resources](./images/rbac-resources.png)
...
{{< hextra/tabs items="dev-pod-reader.yaml,dev-pod-reader-binding.yaml" >}}
...
{{< hextra/tabs items="secret-reader.yaml,secret-reader-binding.yaml" >}}
...
When we create a Pod, it automatically creates a ServiceAccount’s token and mounts it into the container’s filesystem at **/run/secrets/kubernetes.io/serviceaccount/token**
{{< /hextra/callout >}}
...
![NetPol](./images/netpol.png)

The example below (from Kubernetes documentation) is more complex. It illustrates the full capabilities of NetworkPolices.

```yaml {filename="networkpolicy.yaml"}
