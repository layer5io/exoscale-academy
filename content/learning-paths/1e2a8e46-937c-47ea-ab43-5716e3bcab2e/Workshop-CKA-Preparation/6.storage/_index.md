...
We should not use a volume for persistent storage; instead we should use PersistentVolume and PersistentVolumeClaim.
...
- **accessModes** specifies how we can mount the volume in the Pod
...
- **persistentVolumeReclaimPolicy** defines what happens to the PersistentVolume after the associated PersistentVolumeClaim is deleted
...
A PersisentVolumeClaim (PVC) is a storage request. It specifies the storage requirements regqrding size and access mode, and is bound to a PersistentVolume that meets those requirements.
...
More information in the official documentation at [https://kubernetes.io/docs/concepts/storage/](https://kubernetes.io/docs/concepts/storage/)
