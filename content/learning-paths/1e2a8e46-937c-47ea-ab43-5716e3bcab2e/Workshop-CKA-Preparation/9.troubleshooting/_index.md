...
The following picture illustrates various ways to configure logging, both at a Node or at the Cluster's level.

**Node level**: the container runtime stores the logs on the Node's filesystem

![Logging](./images/logging-node-level.png)

**Cluster level (1)** the container directly ships logs to a centralized logging system

![Logging](./images/logging-cluster-level-1.png)

**Cluster level (2)**
  * the container generates its logs on stdout/stderr
  * the container runtime stores the logs on the Node's filesystem
  * an external process (usually deployed as a DeamonSet) reads these logs and ships them to a centralized system

![Logging](./images/logging-cluster-level-2.png)

**Cluster level (3)** 
  * a sidecar container is used to get the logs of the main container and to send them to stdout/stderr
  * the container runtime stores the logs on the Node's filesystem
  * an external process ships them to a centralized system

![Logging](./images/logging-cluster-level-3.png)
...
Accessing the logs of the control plane components does not work on a managed Kubernetes cluster.
{{< /hextra/callout >}}
...
![Prometheus Stack](./images/monitoring-prometheus.png)

### Metrics server

The metrics-server is a lightweigh component that is not installed by default in Kubernetes. It gets CPU / RAM usage in real time but does not store history. Other resources, such as HorizontalPodAutoscaler (HPA), use it to increase/decrease the number of Pods based on resource consumption.
...
{{< hextra/callout type="warning" >}}
Mirror Pods are read-only, we can’t modify nor delete them using kubectl. To alter a mirror Pod, we must change its YAML specification in **/etc/kubernetes/manifests**. 
{{< /hextra/callout >}}
...
![Application not reachable](./images/application-failure-2.png)
...
$ sudo journalctl -u kubelet | less
sudoMar 31 09:49:17 controlplane kubelet[72558]: E0331 09:49:17.443398   72558 pod_workers.go:919] "Error syncing pod, skipping" err="failed to \"StartContainer
\" for \"kube-apiserver\" with CrashLoopBackOff: \"back-off 2m40s restarting failed container=kube-apiserver pod=kube-apiserver-controlplane_kube-system(137
9f6cdef52f9b598e745122eb20d6f)\"" pod="kube-system/kube-apiserver-controlplane" podUID=1379f6cdef52f9b598e745122eb20d6f
Mar 31 09:49:18 controlplane kubelet[72558]: E0331 09:49:18.426742   72558 kubelet_node_status.go:460] "Error updating node status, will retry" err="error g
etting node \"controlplane\": Get \"https://194.182.171.68:6443/api/v1/nodes/controlplane?timeout=10s\": context deadline exceeded"
…
```
...
worker1         NotReady   <none>                 5d19h   v1.32.2   <- This Node does not seems to work
...
The result above indicates that the kubelet process running on worker1 has stopped posting the Node' status, which might indicate that process no longer runs. In that case, we can check the status of the kubelet systemd service on worker1.
